...
describe('canonical agent CLI', () => {
	...
	test('discovers operation names and formats only on demand', async () => {
		...
		expect(operations.map(({ name }) => name)).toEqual([
			...
			'runs.retry_cycle_question', 'runs.retry_verification', 'runs.cancel', 'runs.abandon', 'runs.ship',
		]);
	...
	test('routes every mutation class with JSON and agent-cli provenance', async () => {
		const cases: Array<[string, Record<string, unknown>, string, string]> = [
			...
			['runs.retry_cycle_question', { projectId: PROJECT_ID, runId: 'run-1', reason: 'Transient resolver failure.' }, 'POST', `/api/projects/${PROJECT_ID}/runs/run-1/retry-cycle-question`],
