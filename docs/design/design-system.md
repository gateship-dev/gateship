...
| A page that has nothing to show yet shows the mark building its stair block by block from the bottom (`PageLoading`) and says what it waits for. A skeleton is for a block that waits inside a page that is already there: a panel, the rows of a table. A page does not wear the shape of content it does not have. Under reduced motion the mark stands still | Behaviour tests |
