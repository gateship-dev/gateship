...
| A table is one object. Its controls, the notes about its rows and its pager live inside its frame, in zones on the cells' own 16px inset: the head zone (one 48px row per `DataTableToolbar`), the notice zone (an alert becomes a band of the frame, never a box inside it), the rows, and the foot zone, which is a card's footer: the same muted band at the same 56px, so a table and the card opened beside it close on one band; on the band the secondary ink takes one step toward the main ink, or it falls under AA on the dark wash. A table that fits the smallest page has no foot. Below `sm` the foot keeps the range and the pager and drops the page size, whose select does not fit a phone's text size. A control that acts on the whole page, a time window or the page's own action, is a `PageToolbar` above every block | Visual gate measurement (no table part outside a table, one left edge per table) with a planted case in the smoke |
| One item of a table opens in the `ItemDrawer` beside it, never inside its row: an expanded row breaks the 40px rhythm, cannot hold a form at a reading width and does not lay out under pinned columns. The row is the trigger and stays marked as current; the item is named in the address (`?finding=`, `?proposal=`, `?cohort=`), so it is a link; the arrow keys walk the rows while it is open and it follows; Escape closes it and gives focus back to the row. Above `xl` it is the `Card` every screen uses, as the second column of the layout at the inspector width and inside the table's own ring: exactly as tall as the table, so the two end on one line, and its head is one 48px line with the name cut short, the height of the table's control row, so the rule under the two heads is one line too; its panel scrolls what does not fit and its actions are the muted band that closes it, nothing held to the window. Its fields keep the kit's height and grow with what is typed. Between `md` and `xl` it is a drawer over the right edge, and below `md` the whole screen; there only its body scrolls and its actions are its bottom edge. It is never modal. Dismissed from the drawer, the item leaves and the drawer moves to the next row, else the previous, else closes. A screen writes no `<dialog>` and no `role="dialog"` of its own | Contract test with planted cases, unit tests of the walk and the successor, a real open, step and close against the service at 1440, 1000 and 390 |
| A finding's row names it in the analyzer's own words, the first line of its evidence, and carries its rule beside the name as an id in the data voice, never as a label: a rule is a code, and a label would capitalise it. The drawer says the name once, in its head, sets the rule above the text, and the text starts at the second line | Behaviour tests |
| A table gets a checkbox column only when it has an action that applies to several rows at once; a checkbox that leads nowhere is a promise the table cannot keep. Selection belongs to the page and is dropped when its rows change, and shift extends it from the row clicked last. While rows are selected the head opens with the selection bar, which pushes the controls down rather than covering them, so the list can still be narrowed. The bulk action asks first with the count and the verb on the button itself, each row is its own request, and the rows the service refused are named in the notice zone with its reason: a bulk action is not a transaction and does not pretend to be one | Unit tests of the range and the per-row outcome, a real run against the service with the dismiss requests intercepted |
| The operator can drag any column but the action one to a width, from an 8px grip on its head's right edge; the keyboard moves the same grip by 8px, 32px with shift, Escape during a drag puts the width back, and a double click returns the column to its own content's width. The first drag reads every column at its content's width, never at the slack the browser's layout gave the primary one, and rounds up, because a column a fraction too narrow breaks its widest value. Widths are this browser's, per table. While they add up to more than the frame the rows scroll inside it: the columns up to the primary one and the action column stay put, with a 1px edge that travels with them, and the view menu offers fit to width (the one dragged last keeps its width) and reset | Unit tests, a drag in the smoke through the real grip |
| A table's search says what it is with a magnifier inside the field and offers a clear once it holds text. The rows follow the field once it has been still for 250ms, so a server-backed table asks once per word and not once per key. The way out of every filter at once exists only while something is applied and says how many things it will undo | Behaviour tests, harness test on the real filter path |
| A filter on one column's values is a facet: its face is the column's name in the button's own ink, because a facet's name is a label and never a hint; chosen values follow the name as tags, and past two they become a count; the popup lists the values with their counts when the source knows them and ends with a way to clear. One choice while the source takes one value per column | Behaviour tests, visual gate contrast |
| A column declares the kind of value it holds and never the face that value wears. `name` is what a human wrote and is the row's subject, the only kind that wraps; `label` is a state that already carries its colour; `code` is machine-issued and read to copy or check, mono and one step down because mono at the row's size reads larger than the sans beside it; `measure` is compared by magnitude, mono with tabular figures and right aligned; `moment` stays sans with tabular figures, because a date is read like a word; `action` is the row's menu, 40px and no text | Contract test with two planted cases, judgment |
...
| The controls row carries what the operator acts on here, never a preference chosen once. The theme and the language are chosen in the global settings, in one card of their own: they apply at once, they are this browser's and not the service's, and the theme offers the system as a third state because no stored choice has always meant following it | Behaviour tests |
...
| A screen has one left edge and one right edge, shared by the controls row and every block under it, at every breakpoint. The outer ring is that edge: cards, groups of figures, tables and queues all carry it. Nothing reserves a scrollbar gutter: a reserved gutter is dead space on a platform whose scrollbars overlay, and it pushed the content 11px inside the controls above it. The controls row, which is the panel's, keeps its buttons 24px from the panel's line on both sides, where the content's edge is The content column owns the scrollbar; where the platform draws a classic one it takes width from the content and not from the controls row above it, so the panel measures that width at runtime and the controls row gives exactly it back on the right, and nothing is reserved where no bar is drawn | Visual gate baselines, a measurement against the service with a classic scrollbar forced, judgment |
...
| One chip says a shortcut, and the kit owns the `<kbd>` it is made of (`KeyChip`). It is an outline in the ink of whatever holds it and carries no colour of its own, because a row changes tint under the pointer and when it is current, where a muted fill measured 4.31:1. The hint's surface is the one variant, with the tooltip's own tokens | Contract test with a planted case, visual gate contrast |
