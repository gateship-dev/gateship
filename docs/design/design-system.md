...
| `Alt+1` shows every project and `Alt+2` to `Alt+9` select the first eight projects, in the switcher's order. `Alt` with an arrow walks the destinations, wrapping at the ends. A recognised combination calls `preventDefault`; an unknown one, an index with no project, or a key pressed inside a field is left to the browser. The visible link is always the fallback. Labels read `⌥` on macOS and `Alt` elsewhere | Behaviour tests |
| A project is its digit, in a 16px square the switcher wears open or collapsed, and every project at once is the wildcard. A menu row leads with the mark that says it is current, in a column that is there either way, and closes with its shortcut, as every platform's menus do. On the rail a hint carries the key: the switcher's own, and the pair that walks the destinations | Behaviour tests, judgment |
