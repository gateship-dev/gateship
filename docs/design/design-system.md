...
| A screen has one left edge and one right edge, shared by the controls row and every block under it, at every breakpoint. The outer ring is that edge: cards, groups of figures, tables and queues all carry it. The content column reserves its scrollbar on the end side only; the controls row, which is the panel's, keeps its buttons 24px from the panel's line on both sides | Visual gate baselines, judgment |
