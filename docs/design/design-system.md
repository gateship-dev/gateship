...
| One item of a table opens in the `ItemDrawer` beside it, never inside its row: an expanded row breaks the 40px rhythm, cannot hold a form at a reading width and does not lay out under pinned columns. The row is the trigger and stays marked as current; the item is named in the address (`?finding=`, `?proposal=`, `?cohort=`), so it is a link; the arrow keys walk the rows while it is open and it follows; Escape closes it and gives focus back to the row. Above `xl` it pushes, as the second column of the layout at the inspector width, inside the table's own ring and as tall as the table, so the two end on one line; its head and its foot are a card's, the foot the same muted band with the same stickiness, and while the page scrolls the drawer the name holds the top of the window and the actions its bottom; between `md` and `xl` it overlays the right edge; below `md` it is the whole screen. It is never modal. Dismissed from the drawer, the item leaves and the drawer moves to the next row, else the previous, else closes. A screen writes no `<dialog>` and no `role="dialog"` of its own | Contract test with planted cases, unit tests of the walk and the successor, a real open, step and close against the service at 1440, 1000 and 390 |
