...
async function retryVerificationDurableRun(
	request: Request,
	runtime: RunRuntime,
	runId: string,
	admit?: () => void,
	...
	try {
		admit?.();
		return Response.json({ ok: true, run: runtime.retryVerificationRun(runId, reason, commandSource(request)) }, { status: 202 });
	} catch (error) {
		const unavailable = error instanceof RuntimeUnavailableError;
		return Response.json({ ok: false, code: unavailable ? 'runtime-unavailable' : 'run-verification-retry-refused', message: error instanceof Error ? error.message : String(error) }, { status: unavailable ? 503 : 409 });
	}
}
