...
function ReportView({ report }: { report: DesignReport }): React.ReactElement {
	...
	return (
		<div className="flex flex-col gap-3">
			...
			{report.overflow.length > 0 ? <details className="text-xs"><summary className="cursor-default">Overflow</summary><ul className="mt-2 grid gap-1 font-mono">{report.overflow.map((finding, index) => <li key={index}>{finding.slot}: {finding.scrollWidth} in {finding.clientWidth}</li>)}</ul></details> : null}
