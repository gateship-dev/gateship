...
export function RegisteredProjectsTable({ props }: { props: AppProps }): React.ReactElement {
	...
	const columns = useMemo<GateshipColumnDef<RegisteredProject>[]>(() => {
		...
		const defs: GateshipColumnDef<RegisteredProject>[] = [
			{ id: 'project', header: overviewCatalog.project, meta: { kind: 'name', className: 'max-w-44 sm:max-w-52' }, cell: ({ row }) => <span className="flex min-w-0 flex-wrap items-center gap-2"><a className={cn(TITLE_LINK_CLASS, 'truncate')} href={`/projects/${encodeURIComponent(row.original.id)}`}>{row.original.name}</a>{row.original.current ? <span className="hidden @xl:inline-flex"><Tag>{catalog.currentBadge}</Tag></span> : null}</span> },
			{ id: 'repository', header: catalog.list.repository, meta: { kind: 'code', className: 'max-w-64 truncate text-muted-foreground', hideBelow: 'md' }, cell: ({ row }) => row.original.repository ?? catalog.repositoryUnknown },
			{ id: 'readiness', header: catalog.readinessLabel, meta: { kind: 'label' }, cell: ({ row }) => <Badge variant={READINESS_TONE[row.original.readiness]}>{catalog.readiness[row.original.readiness]}</Badge> },
			{ id: 'activity', header: overviewCatalog.activity, meta: { kind: 'label', className: 'max-w-56', hideBelow: 'sm' }, cell: ({ row }) => { const entry = entryOf(row.original); return entry === undefined ? null : <ProjectActivity catalog={overviewCatalog} entry={entry} locale={props.locale} />; } },
			{ id: 'settings', header: () => <span className="sr-only">{catalog.list.settings}</span>, meta: { kind: 'action', label: catalog.list.settings }, cell: ({ row }) => <a className={TEXT_LINK_CLASS} href={`/projects/${encodeURIComponent(row.original.id)}/settings`}>{catalog.list.settings}</a> },
		];
