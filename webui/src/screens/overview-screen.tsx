		...
		<div className="rounded-lg border">
			<Table>
				...
				<TableBody>{overview.projects.map((entry) => <TableRow key={entry.project.id}>
					<TableCell className="max-w-52"><span className="flex min-w-0 flex-wrap items-center gap-2"><a className={cn(TITLE_LINK_CLASS, 'truncate')} href={`/projects/${encodeURIComponent(entry.project.id)}`}>{entry.project.name}</a>{entry.project.current ? <Badge variant="info">{projectCatalog.currentBadge}</Badge> : null}</span></TableCell>
