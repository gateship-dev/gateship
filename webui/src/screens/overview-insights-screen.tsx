...
function InsightsData({ history, catalog, labels, locale, filter, sortBy, sortDirection }: { history: HistoricalOverviewView; catalog: OverviewInsightsCatalog; labels: Record<string, string>; locale: AppProps['locale']; filter?: string; sortBy?: InsightsQuery['cohortSortBy']; sortDirection?: InsightsQuery['cohortSortDirection'] }): React.ReactElement {
	...
	// GSHIP-889: the known subtotal below sums every run's known portion, partial included -- shown beside the coverage marker rather than read as if every counted run were fully priced.
	const knownCostValue = history.knownCostUsd === null || history.runsByCostCoverage === undefined || history.runsByCostCoverage.complete >= history.totalRuns
		? formatCost(history.knownCostUsd)
		: `${formatCost(history.knownCostUsd)} · ${LOCALE_CATALOG[locale].runsOperational.cost.partialCoverage}`;
	return <><CohortComparisonSection cohorts={history.cohorts} labels={labels} />
		<CardGrid className="sm:grid-cols-2 xl:grid-cols-4" compact equalHeight><Stat label={catalog.runs} value={history.totalRuns} /><Stat label={catalog.shipped} value={history.runsByOutcome.shipped} /><Stat label={catalog.dispatchToMerge} value={formatDuration(history.medianDispatchToMergeMs)} /><Stat label={catalog.costCoverage} value={costCoverageValue} /></CardGrid>
		...
		<MetricSection title={catalog.economy}><MetricList items={[[catalog.knownCost, knownCostValue], [catalog.apiEquivalent, knownCostValue], [catalog.inputTokens, formatTokens(history.reportedTokens.inputTokens, locale)], [catalog.outputTokens, formatTokens(history.reportedTokens.outputTokens, locale)]]} /><p className="mt-4 text-muted-foreground text-xs">{catalog.subscriptionSeparate}</p></MetricSection>
