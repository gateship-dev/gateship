...
export function OverviewInsightsSurface({ props }: { props: AppProps }): React.ReactElement {
	...
	const update = (changes: Partial<typeof query>): void => { const next = updatedInsightsQuery(query, changes); browserRuntime().history?.pushState(null, '', insightUrl(next.window, next.projectId, next.cohortOffset)); setQuery(next); };
