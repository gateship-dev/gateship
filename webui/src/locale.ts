...
export interface OverviewCatalog {
	title: string;
	description: string;
	navigation: { label: string; now: string; runs: string; queues: string; insights: string };
	loading: string;
	empty: string;
	partial: string;
	error: string;
...
export interface RunInspectorCatalog {
	...
	specFacts: {
		...
		dispatches: string; guidance: string; authorization: string;
...
export interface RunsOperationalCatalog {
	cost: {
		...
		/** GSHIP-889: shown in place of a model row's cost when no invocation of it ever reported a price -- never a fabricated $0. */
		unknownCost: string;
		/** GSHIP-889: appended beside a known subtotal once at least one invocation in the run never reported a price, so a partial total is never read as the run's whole cost. */
	...
	activity: {
		...
		returnToLive: string;
		roleLabels: Readonly<Record<'executor' | 'reviewer' | 'orchestrator' | 'operator' | 'agent-cli' | 'unknown' | 'runtime', string>>;
...
export interface LocaleCatalog {
	...
	overviewInsights: OverviewInsightsCatalog;
...
export const LOCALE_CATALOG = {
	'en-US': {
		...
		runInspector: {
			...
			correctionRounds: (executor, ci, decision, orchestrator, indeterminate) => {
				...
				const parts = [
					...
					`${orchestrator} after ${orchestrator === 1 ? 'an orchestrator continue' : 'orchestrator continues'}`,
				];
				if (indeterminate > 0) {
		...
		runsWorkflow: {
			signals: {
				...
				corrections: (roundCount, runCount, executor, ci, decision, orchestrator, indeterminate) =>
					`${roundCount} ${roundCount === 1 ? 'round' : 'rounds'} across ${runCount} ${runCount === 1 ? 'run' : 'runs'}: ${executor} automatic${ci === 0 ? '' : `, ${ci} from CI correction`}, ${orchestrator} after ${orchestrator === 1 ? 'an orchestrator continue' : 'orchestrator continues'}, ${decision} after ${decision === 1 ? 'a human decision' : 'human decisions'}${indeterminate === 0 ? '' : `, ${indeterminate} with indeterminate origin`}`,
		...
		settings: {
			...
			updates: { title: 'Gateship updates', description: 'Checks official releases at most daily and applies a verified native binary only while the project is idle.', label: 'Install verified native updates automatically', guidance: 'Fixed cadence: daily. Runs, preserved waiting states, diagnostics, containers, and source checkouts are never updated in place.', installed: 'Installed', available: 'Available', unknown: 'unknown', statusLabels: { success: 'success', rollback: 'rollback', failed: 'failed', 'check-failed': 'check-failed', deferred: 'deferred' }, result: (previous, target, at) => `${previous} → ${target} at ${at}` },
	...
	'pt-BR': {
		...
		runInspector: {
			...
			correctionRounds: (executor, ci, decision, orchestrator, indeterminate) => {
				...
				const parts = [
					...
					`${orchestrator} após continue do orquestrador`,
				];
				if (indeterminate > 0) {
				...
				specFacts: {
				title: 'Fatos da especificação', description: 'Perfil imutável e fatos reconstituídos da run, sem nota ou julgamento.', version: 'Versão', fingerprint: 'Fingerprint', specCounts: 'Contagens da spec', corrections: 'Correções', questions: 'Perguntas do ciclo', reconciliations: 'Reconciliações', dispatches: 'Despachos ao provider', guidance: 'Canais da orientação', authorization: 'Evidência de autorização', dispatchCounts: (executor, reviewer, orchestrator, total, unknown) => `executor ${executor}, revisor ${reviewer}, orquestrador ${orchestrator}, total ${total}, desconhecido ${unknown}`, guidanceCounts: (web, agentCli, other, unknown) => `web ${web}, agent-cli ${agentCli}, outro ${other}, desconhecido ${unknown}`, authorizationCounts: (observed, absent, unknown) => `observada ${observed}, ausente ${absent}, desconhecida ${unknown}`, unknown: 'desconhecido', durationTitle: 'Duração da run por fase', wallTime: 'Tempo total', unassigned: 'Não atribuído', reconciliation: 'Reconcilia com o tempo total', entries: (count) => `${count} entrada${count === 1 ? '' : 's'}`, yes: 'sim', no: 'não', phaseLabels: { queued: 'Na fila', working: 'Trabalho ativo', verify: 'Verificação', review: 'Revisão', 'full-verify': 'Verificação completa', shipping: 'Entrega', 'waiting-provider': 'Espera do provedor', 'waiting-user': 'Espera do operador' },
				counts: (acceptance, boundaries, verify, evidence) => `acceptance ${acceptance}, boundaries ${boundaries}, verify ${verify}, evidence ${evidence}`,
		...
		runsWorkflow: {
			signals: {
				...
				corrections: (roundCount, runCount, executor, ci, decision, orchestrator, indeterminate) =>
					`${roundCount} ${roundCount === 1 ? 'rodada' : 'rodadas'} em ${runCount} ${runCount === 1 ? 'execução' : 'execuções'}: ${executor} automática${executor === 1 ? '' : 's'}${ci === 0 ? '' : `, ${ci} de Correção de CI`}, ${orchestrator} após continue do orquestrador, ${decision} após ${decision === 1 ? 'uma decisão humana' : 'decisões humanas'}${indeterminate === 0 ? '' : `, ${indeterminate} de origem indeterminada`}`,
